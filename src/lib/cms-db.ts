...
export async function getPrograms(): Promise<Program[]> {
  try {
    const rows = await prisma.program.findMany({ orderBy: [{ category: 'asc' }, { title: 'asc' }] });
    if (rows.length) {
      return rows.map((row) => ({
        id: row.id,
        title: row.title,
...
export async function getNotices(): Promise<Notice[]> {
  try {
    const rows = await prisma.notice.findMany({ orderBy: [{ pinned: 'desc' }, { date: 'desc' }, { createdAt: 'desc' }] });
...
export async function getDocuments(): Promise<AccreditationDocument[]> {
  try {
    const rows = await prisma.document.findMany({ orderBy: [{ updatedAt: 'desc' }, { title: 'asc' }] });
...
export async function getFaculty(): Promise<FacultyMember[]> {
  try {
    const rows = await prisma.faculty.findMany({ orderBy: [{ spotlight: 'desc' }, { name: 'asc' }] });
    if (rows.length) return rows.map((row) => ({ id: row.id, name: row.name, slug: row.slug, photo: row.photo || undefined, designation: row.designation || undefined, department: row.department || undefined, qualification: row.qualification || undefined, experience: row.experience || undefined, expertise: row.expertise || undefined, bio: row.bio || undefined, verification: row.bio || undefined, publications: parseJson<string[]>(row.publications, []), contactEmail: row.contactEmail || undefined, contactPhone: row.contactPhone || undefined, contactVisible: row.contactVisible, spotlight: row.spotlight, status: row.status }));
  } catch {}
...
export async function readCMSCollection<T = unknown>(collection: CMSCollection): Promise<T> {
  switch (collection) {
    ...
    case 'media': return (await prisma.mediaAsset.findMany({ orderBy: { createdAt: 'desc' } }).catch(() => readFallback('media', []))) as T;
    case 'forms': return (await prisma.formSubmission.findMany({ orderBy: { createdAt: 'desc' }, take: 500 }).catch(() => [])) as T;
    case 'admissions': return (await prisma.admissionLead.findMany({ orderBy: { createdAt: 'desc' }, take: 500 }).catch(() => [])) as T;
    case 'recruiters': return (await prisma.recruiterInquiry.findMany({ orderBy: { createdAt: 'desc' }, take: 500 }).catch(() => [])) as T;
    case 'alumni': return (await prisma.alumniRegistration.findMany({ orderBy: { createdAt: 'desc' }, take: 500 }).catch(() => [])) as T;
    case 'contacts': return (await prisma.contactInquiry.findMany({ orderBy: { createdAt: 'desc' }, take: 500 }).catch(() => [])) as T;
    case 'events': return (await prisma.event.findMany({ orderBy: { createdAt: 'desc' } }).catch(() => readFallback('events', []))) as T;
    case 'blogs': return (await prisma.blogPost.findMany({ orderBy: { createdAt: 'desc' } }).catch(() => readFallback('blogs', []))) as T;
    case 'careers': return (await prisma.jobOpening.findMany({ orderBy: { createdAt: 'desc' } }).catch(() => readFallback('careers', []))) as T;
...
export async function readAdminCollection<T = unknown>(collection: CMSCollection): Promise<T> {
  switch (collection) {
    case 'site': return [await getSiteSettings()] as T;
    case 'homepage-sections': return (await getHomepageSections()) as T;
    case 'pages': return (await prisma.page.findMany({ orderBy: { slug: 'asc' } })) as T;
    case 'programs': return (await prisma.program.findMany({ orderBy: [{ category: 'asc' }, { title: 'asc' }] })).map((row: any) => ({ ...row, rules: parseJson(row.rules, []), documents: parseJson(row.documents, []), faqs: parseJson(row.faqs, []), facultyIds: parseJson(row.facultyIds, []) })) as T;
    case 'notices': return (await prisma.notice.findMany({ orderBy: [{ pinned: 'desc' }, { date: 'desc' }, { createdAt: 'desc' }] })).map((row: any) => ({ ...row, url: row.externalUrl || row.documentUrl || '' })) as T;
    case 'documents': return (await prisma.document.findMany({ orderBy: [{ updatedAt: 'desc' }, { title: 'asc' }] })).map((row: any) => ({ ...row, file: row.fileUrl, tags: parseJson(row.tags, []) })) as T;
    case 'faculty': return (await prisma.faculty.findMany({ orderBy: [{ spotlight: 'desc' }, { name: 'asc' }] })).map((row: any) => ({ ...row, publications: parseJson(row.publications, []) })) as T;
    case 'media': return (await prisma.mediaAsset.findMany({ orderBy: { createdAt: 'desc' } })).map((row: any) => ({ ...row, tags: parseJson(row.tags, []) })) as T;
    case 'value-added-courses': return (await getValueAddedCourses()) as T;
    case 'forms': return (await prisma.formSubmission.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })) as T;
    case 'admissions': return (await prisma.admissionLead.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })) as T;
    case 'recruiters': return (await prisma.recruiterInquiry.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })) as T;
    case 'alumni': return (await prisma.alumniRegistration.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })) as T;
    case 'contacts': return (await prisma.contactInquiry.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })) as T;
    case 'events': return (await prisma.event.findMany({ orderBy: { createdAt: 'desc' } })) as T;
    case 'blogs': return (await prisma.blogPost.findMany({ orderBy: { createdAt: 'desc' } })) as T;
    case 'careers': return (await prisma.jobOpening.findMany({ orderBy: { createdAt: 'desc' } })) as T;
