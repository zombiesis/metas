...
export async function PUT(request: NextRequest) {
  ...
  const row = await prisma.siteSetting.upsert({ where: { key: 'site' }, create: { key: 'site', label: 'Site settings', group: 'global', value: JSON.stringify(data), updatedBy: auth.session!.username }, update: { value: JSON.stringify(data), updatedBy: auth.session!.username } });
